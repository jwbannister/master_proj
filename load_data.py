...
npv_file = pd.ExcelFile(file_path + "NPV INPUT.xlsx")
# what years are MP steps to be implemented?
step_years = npv_file.parse(sheet_name="mp_schedule (INPUT)", header=0)
# DCM costs
dcm_costs = npv_file.parse(sheet_name="dcm_costs (INPUT)", header=0)
dcm_costs.dropna(inplace=True)
# mapping between MP habitats and DCMs
hab2dcm = npv_file.parse(sheet_name="hab2dcm (INPUT)", header=0, \
        usecols="A,B,C,D")
# calculation factors (escalation rate, etc.)
factors = npv_file.parse(sheet_name="factors (INPUT)", header=None, \
        usecols=[0, 1], index_col=0, convert_float=False).T
...
npv_summary['capital_cash'] = npv_summary['capital_esc'] *\
        (1 - factors['finance_ratio'].item())
npv_summary['capital_finance'] = npv_summary['capital_esc'] *\
        factors['finance_ratio'].item()
...
npv_summary['pumped_water'] = \
        ...
        (npv_summary['year'] >= factors['pump_start_year'].item())
...
npv_summary['avoided_cost'] = (npv_summary['avoided_water_purchase'] * \
        mwd_costs)/1000000
npv_summary['npv_avoided_cost'] = npv_summary['avoided_cost'] /\
        ((1 + factors['discount_rate'].item())**(npv_summary['year'] - \
        factors['npv_year'].item()))
npv_summary['net_return'] = npv_summary['avoided_cost'] - npv_summary['cost']
npv_summary['npv_net_return'] = npv_summary['net_return'] /\
        ((1 + factors['discount_rate'].item())**(npv_summary['year'] - \
        factors['npv_year'].item()))
npv_summary = npv_summary[['year', 'capital', 'o&m', 'capital_esc', \
        ...
        'avoided_cost', 'net_return', 'npv_avoided_cost', 'npv_net_return']]

writer = pd.ExcelWriter(file_path + "NPV OUTPUT.xlsx")
npv_summary.to_excel(writer, 'NPV_summary (OUTPUT)', index=False)
step_years.to_excel(writer, 'mp_schedule (INPUT)', index=False)
dcm_costs.to_excel(writer, 'dcm_costs (INPUT)', index=False)
hab2dcm.to_excel(writer, 'hab2dcm (INPUT)', index=False)
factors.T.to_excel(writer, 'factors (INPUT)', index=True)
writer.save()
