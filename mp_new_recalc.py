...
mp_name = "Master_proj_20180321.xlsm"
...
mp_new = mp_file.parse(sheet_name="MP_new", header=1, \
        usecols="Q,R,U,V,W,X,AA", names=mp_new_names, \
...
mp_new_wd = mp_file.parse(sheet_name="MP_new", header=1, \
        usecols="Q,AA,AN,BB,BP,CD", converters={'Step':int}, \
        names=[mp_new_names[i] for i in [0, 6, 2, 3, 4, 5]]).dropna(how='any')
mp_new_wd_peracre = mp_file.parse(sheet_name="MP_new", header=1, \
        usecols="Q,AA,AG,AU,BI,BW", converters={'Step':int}, \
        names=[mp_new_names[i] for i in [0, 6, 2, 3, 4, 5]]).dropna(how='any')
# pull surface breakdown by habitat description
breakdown_names = ["hab_id", "sat", "pond", "veg1", "veg2", "veg3", \
        "veg4", "dry"]
dcm_breakdown = mp_file.parse(sheet_name="DCMs", header=4, \
        usecols="F,J,K,L,M,N,O,P", names=breakdown_names).dropna(how='any')
dcm_breakdown['veg'] = dcm_breakdown.loc[:, 'veg1':'veg4'].sum(axis=1)
dcm_breakdown.drop(columns=["veg1", "veg2", "veg3", "veg4"], inplace=True)
dcm_breakdown['checksum'] = dcm_breakdown.loc[:, 'sat':'veg'].sum(axis=1)
dcm_breakdown[dcm_breakdown['checksum']!=1]
# get list of habitat designations actually used in MP workbook
id_dict = pd.Series(hab2dcm.hab_id.values, index=hab2dcm.mp_name).to_dict()
hab_ids = mp_new.loc[:, 'dcm_base':'dcm_step5'].replace(id_dict)
hab_unique = set(hab_ids.values.ravel())
mp_breakdown = dcm_breakdown[dcm_breakdown['hab_id'].isin(hab_unique)]

# comparison of water duty for each habitat in mp_new
hab =pd.Series({})
wd =pd.Series({})
for col in ['dcm_base', 'dcm_dwm', 'dcm_step0', 'dcm_step5']:
    hab = hab.append(mp_new[col])
    wd = wd.append(mp_new_wd_peracre[col])
a = pd.concat([hab, wd], axis=1)
a.columns = ['hab', 'wd']
a.groupby('hab').std()
a.groupby('hab').count()==1
a[a['hab']=='SFP']
